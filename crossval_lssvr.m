function[soln_w,soln_reg,soln_biassvr,soln_alphasvr,soln_mse,fold_error] = crossval_lssvr(kfold,A_fold,b_fold,reg)
tuberadius=zeros(1,1);
...
%     
%         %Operation
         [wlssvrcv,alphalssvrcv,elssvrcv,biaslssvrcv]=lssvr(A_train,b_train,reg);
         
         %[0] indicates tube radius not involved
         [err_trf]=svr_error(A_train,b_train,wlssvrcv,biaslssvrcv,[0],reg); 
         
         %[0] indicates tube radius not involved
         [err_valf]=svr_error(A_val,b_val,wlssvrcv,biaslssvrcv,[0],reg);
%         
...
%         soln_biassvr(:,i)=biassvrcv(min_ind);
    ...
    soln_w=wlssvrcv(:,min_ind);
    soln_alphasvr=alphalssvrcv(:,min_ind);
    soln_mse=min_mse;
    soln_biassvr=biaslssvrcv(min_ind);
%     %PLOT
%     figure;
